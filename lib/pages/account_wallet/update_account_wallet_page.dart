...
class UpdateAccountWalletPage extends StatefulWidget {
  const UpdateAccountWalletPage({
    super.key,
    required this.dataToUpdate,
  });
  final Map<String, dynamic> dataToUpdate;
  @override
  State<UpdateAccountWalletPage> createState() => _UpdateAccountWalletPageState();
}

class _UpdateAccountWalletPageState extends State<UpdateAccountWalletPage> {
  //mainly variable
  ...
  Map<String, dynamic> currentBankInfo = {};
  String accountWalletType = '';
  String nameChosenAccountWalletType = '';
  bool alertNullNameWallet = false;
  @override
  void initState() {
    print(widget.dataToUpdate['money_type_information']['name']);
    print(widget.dataToUpdate);
    nameChosenAccountWalletType = widget.dataToUpdate['money_type_information']['name'];
    resultChosenMoneyAccountType = {
      "name": widget.dataToUpdate['money_type_information']['name'],
      "icon": widget.dataToUpdate['money_type_information']['icon'],
    };
    //init all value
    moneyTextFieldController.text = widget.dataToUpdate['account_balance'][r'$numberDecimal'];
    creditTextFieldController.text = widget.dataToUpdate['credit_limit_number'][r'$numberDecimal'];
    nameWalletTextFieldController.text = widget.dataToUpdate['name'];
    idChosenBank = widget.dataToUpdate['select_bank'] == '' ? 0 : int.parse(widget.dataToUpdate['select_bank']);
    idMoneyAccountType = widget.dataToUpdate['money_account_type_id']??'';
    describeTextFieldController.text = widget.dataToUpdate['description'];
    isReport = widget.dataToUpdate['report'] == '1' ? true: false;
    super.initState();
  }
                        ...
                        children: [
                          Consumer<AppProvider>(
                            builder: (context, value, child) {
                              for(var item in value.bank){
                                if(item['id'] == idChosenBank){
                                  resultChosenBank['icon'] = item['logo'] ?? '';
                                  resultChosenBank['shortName'] = item['short_name']?? '';
                                }
                              }
                              return MyListTile(
                                  leading: CircleAvatar(
                                    ...
                                    child: ClipOval(
                                      child: Image.network(
                                        resultChosenBank.isNotEmpty ? resultChosenBank['icon'] :
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    Map<String, String> dataToPass = {
                      "money_account_id" : widget.dataToUpdate['_id'],
                      "name":"",
                      "account_balance": '0',
                      "money_account_type_id":"",
                      "description":"",
                      "report": isReport == true? '1': '0',
                      "select_bank":'0',
                    ...
                    if(nameWalletTextFieldController.text.isNotEmpty &&
                      ...
                      dataToPass.update('report', (value)=> isReport == true ? '1': '0');
                      if(
                          nameChosenAccountWalletType.toLowerCase().contains('tài khoản ngân hàng') ||
                          nameChosenAccountWalletType.toLowerCase().contains('thẻ tín dụng')
                      ){
                      ...
                      result = await Provider.of<UserProvider>(context, listen: false).updateMoneyAccount(dataToPass);
